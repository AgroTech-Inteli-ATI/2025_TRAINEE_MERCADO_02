---
slug: /sprint-1/mapa-conceitual
title: Mapa Conceitual da Fazenda
sidebar_position: 3
---

# Mapa Conceitual da Fazenda Modelo

Esta seção apresenta o **mapa conceitual da Fazenda Modelo Agrominas**, elaborado a partir dos arquivos vetoriais fornecidos pelo parceiro (formato `.kml`) e da visualização territorial no **Google Earth Pro**.  
O objetivo é representar a **projeção de uso do solo** e a **distribuição de hectares por sistema produtivo**, considerando as práticas regenerativas definidas nos [Critérios de Divisão](2-criterios-de-divisao).

---

## 1. Base Territorial e Fonte de Dados

A base geoespacial foi disponibilizada pela **Agrominas Fertilizantes** e contém as delimitações das áreas produtivas identificadas no Google Earth Pro.  
Cada polígono foi classificado com base no tipo de uso proposto (Café, Abacate, Pecuária Regenerativa e Áreas de Preservação).

**Arquivos utilizados:**
- `Fazenda_Agrominas.kml` — polígonos principais da propriedade (áreas A a G);
- `Limite_Territorial.kml` — contorno geral da fazenda.

**Localização aproximada:**  
Sul de Minas Gerais, município de São Bento Abade (MG), região de Lavras e Varginha.  
Coordenadas centrais: **23K 463389.79 m E / 7580621.7 m S**  
Altitude média: **~939 m**  
Fonte das imagens: **Maxar Technologies (agosto/2023)**.

---

## 2. Projeção de Uso do Solo

Com base na análise geográfica, as áreas foram agrupadas em **cinco sistemas principais**, respeitando o relevo, drenagem e uso atual do solo:

| Uso do Solo | Áreas Correspondentes | Área Total (ha) | Percentual (%) | Observações |
| :--- | :--- | ---: | ---: | :--- |
| **Sistema 1: SAF (Café + Abacate)** | A, B, C | 17,9 | 35% | Sistemas agroflorestais com integração de culturas perenes. |
| **Sistema 2: SSP (Pecuária + Nativas)** | D, E | 9,55 | 19% | Pastagens regenerativas com corredores ecológicos. |
| **Sistema 3: ILPF (Integração Total)** | F | 3,55 | 7% | Integração Lavoura-Pecuária-Floresta experimental. |
| **Área de Preservação (APP + Reserva)** | G + margens | 5,43 | 10% | Preservação de nascentes e mata ciliar. |
| **Infraestrutura (Sede, Estradas, Galpões)** | Trechos lineares e zona central | 15,0 | 29% | Inclui acessos internos, sede e áreas de circulação. |
| **TOTAL** | — | **51,43 ha** | **100%** | — |

*(Valores aproximados, obtidos via medição poligonal no Google Earth Pro.)*

---

## 3. Representação Visual (Protótipo)

Abaixo, o mapa conceitual da fazenda, com a sobreposição das áreas delimitadas (A–G) sobre a imagem de satélite:

![Mapa Conceitual da Fazenda Modelo](./img/mapa_conceitual_fazenda.jpg)

**Legenda:**
-  **Verde:** SAF – Café + Abacate  
-  **Azul:** SSP – Pecuária Regenerativa  
-  **Amarelo:** ILPF Experimental  
-  **Vermelho:** Infraestrutura e Sede  
-  **Cinza-esverdeado:** APP e Reserva Legal  

---

## 4. Interpretação Conceitual

O **Mapa Conceitual** não representa apenas a divisão física da fazenda, mas sim a **integração funcional entre os sistemas produtivos**.  
Ele orienta as próximas etapas de planejamento — modelagem financeira e certificações — garantindo coerência entre **uso do solo**, **objetivos de sustentabilidade** e **aplicação dos insumos Agrominas**.

---

## 5. Próximos Passos

- Validação dos limites e altitudes no **QGIS**;  
- Geração de mapa de declividade e drenagem para ajuste de zonas produtivas;  
- Exportação do arquivo consolidado em `.kml` e `.geojson` para integração ao **protótipo visual (Sprint 3)**.
